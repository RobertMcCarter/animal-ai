...
class DataAnnotatorUI:
    ...
    def __init__(self):
        ...
        self.filemenu.add_command(
            label="Open folder...", command=self.promptUserForFolderToProcess
        )
        self.filemenu.add_command(
            label="Save annotations", command=self._saveAnnotations
        )
    ...
    def promptUserForFolderToProcess(self):
        ...
        fileName: str = filedialog.askopenfilename(  # type: ignore
            parent=self.root, title="Select any file in the folder to process..."
        )
        if not fileName:
            return
    ...
    def moveToImage(self, index: int):
        ...
        """
        assert self._manager
        if not self._manager.isValidIndex(index):
            return
    ...
    def _updateImage(self):
        ...
        if self._canvasImageId == 0:
            ...
            self._canvasImageId = self._canvas.create_image(
                0, 0, image=tkImage, anchor=tk.NW
            )
        else:
    ...
    def _startAutoMoveTimer(self):
        ...
        self._autoMoveTimer = threading.Timer(
            NEXT_IMAGE_SECONDS, moveToNextImageAndRestartTimer
        )
