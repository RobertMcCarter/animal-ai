...
from .region2d import Size2d, ScaledRegion2d
...
class AnnotatedImage:
    ...
    def scaleImage(self, scale:float) -> Image.Image:
        ...
        self._scaledImage = imageUtils.scaleImage(self.image, scale)
        self._scale = scale
        return self._scaledImage
    ...
    def scaleImageForSize(self, targetSize: Size2d) -> Union[float, None]:
        ...
        if imageUtils.isAlreadyScaledCorrectly(self.scaledImage, targetSize):
            assert self.scaledImage
            return None

        # Okay, now we can scale the image
        scale = imageUtils.calculateImageScale(self.image, targetSize)
        with Timer("scaling image"):
            self.scaleImage(scale)

        # We now need to re-scale selected regions
        for region in self._regions:
            region.screenRegion = region.imageRegion.scale(scale)
        return scale
