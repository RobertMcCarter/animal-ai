...
from .region2d import Size2d, Region2d, ScaledRegion2d


def clearImagesOutsideRange( annotatedImages:List[AnnotatedImage],
                             currentIndex:int,
                             keepPrevious:int=10,
                             keepNext:int=10 ) -> None:
...
class AnnotatedImagesManager:
    ...
    def addActiveRegion(self) -> None:
        ...
        """
        if self.activeRegion is None: return
        self.activeRegion.canvasRectId = 0      # It no longer belongs to that canvas rectangle
    ...
    def scanForTaggedIndex(self, direction) -> Union[int, None]:
